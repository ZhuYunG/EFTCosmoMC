...
DEFAULT_SAMPLES_DIR = "/Volumes/My Passport/Horndeski_samples_py_onlybackground_a00_1"
# DEFAULT_SAMPLES_DIR = "/Users/dcz/Documents/eft_code/EFTCosmoMC/EFTCAMB/Horndeski_samples_py_onlybackground_4"
DEFAULT_OUT_DIR = (
    "/Volumes/My Passport/prior/Horndeski_samples_piror_max_a00_1"
)
GRAV_COUPLING_MIN = 0.9
GRAV_COUPLING_MAX = 1.1
...
def parse_bool(value: str) -> bool:
    ...
    if normalized in {"false", "f", "0", "no", "n"}:
        return False
    raise argparse.ArgumentTypeError("Expected True or False for boolean option.")


def append_run_readme(out_dir: str, args: argparse.Namespace, samples_dirs: List[str]) -> str:
    ...
    for key in sorted(vars(args)):
        lines.append(f"  {key}: {format_arg_value(getattr(args, key))}")
    lines.append(f"Resolved samples_dirs: {format_arg_value(samples_dirs)}")
    lines.append(f"Resolved out_dir: {format_arg_value(out_dir)}")
...
def read_first_data_row(path: str) -> Tuple[List[str] | None, str | None]:
    try:
        with open(path, "r", encoding="utf-8") as handle:
            for line in handle:
                stripped = line.strip()
                ...
                if stripped.startswith("#"):
                    continue
                return stripped.split(), None
    except OSError as exc:
        return None, f"failed to read first data row: {exc}"
    return None, "no data rows"

def read_omega0_value(path: str) -> Tuple[float | None, str | None]:
    ...
    omega_idx = token_map.get(normalize_token("Omega"))
    if omega_idx is None:
        return None, "missing Omega column"

    row_tokens, row_error = read_first_data_row(path)
    if row_error is not None:
        return None, row_error
    if row_tokens is None:
        return None, "no data rows"
    if omega_idx >= len(row_tokens):
        return None, "data row has fewer columns than header"
    try:
        omega0 = float(row_tokens[omega_idx])
    except ValueError:
        return None, "invalid Omega value in first data row"
    return omega0, None
...
def compute_spike_scores(target_matrix: np.ndarray, _a_ref: np.ndarray) -> np.ndarray:
    abs_vals = np.abs(target_matrix)
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default=True,
        help=(
            "Filter samples by 1/(1+Omega_0) in [0.9, 1.1] using the first "
            "data row (a=1). Set to False to disable."
        ),
    )
    parser.add_argument(
        "--n-bin",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Fraction of curves to drop by max-|w| score (0 disables).",
    )
    parser.add_argument(
        "--winsorize",
        action="store_true",
    ...
    if missing_dirs:
        ...
        for path in missing_dirs:
            print(f"  {path}")
        return 1
    readme_path = append_run_readme(out_dir, args, samples_dirs)
    ...
    print_skips = not args.grav_coupling

    if args.grav_coupling:
        print(
            "Grav coupling filter: enabled "
            f"(range=[{GRAV_COUPLING_MIN}, {GRAV_COUPLING_MAX}])"
        )
    else:
        print("Grav coupling filter: disabled")

    for path in files:
        if args.grav_coupling:
            omega0, omega_reason = read_omega0_value(path)
            if omega_reason is not None:
                reason = f"grav_coupling check failed: {omega_reason}"
                skip_reasons_step0.append((path, reason))
                if print_skips:
                    print(f"SKIP step0: {path} -> {reason}")
                continue
            if not np.isfinite(omega0):
                reason = "grav_coupling Omega_0 is not finite"
                skip_reasons_step0.append((path, reason))
                if print_skips:
                    print(f"SKIP step0: {path} -> {reason}")
                continue
            denom = 1.0 + omega0
            if denom == 0.0:
                reason = "grav_coupling Omega_0=-1 leads to division by zero"
                skip_reasons_step0.append((path, reason))
                if print_skips:
                    print(f"SKIP step0: {path} -> {reason}")
                continue
            grav_value = 1.0 / denom
            if not np.isfinite(grav_value):
                reason = "grav_coupling 1/(1+Omega_0) is not finite"
                skip_reasons_step0.append((path, reason))
                if print_skips:
                    print(f"SKIP step0: {path} -> {reason}")
                continue
            if not (GRAV_COUPLING_MIN <= grav_value <= GRAV_COUPLING_MAX):
                reason = (
                    f"grav_coupling={grav_value:.6f} outside "
                    f"[{GRAV_COUPLING_MIN}, {GRAV_COUPLING_MAX}]"
                )
                skip_reasons_step0.append((path, reason))
                if print_skips:
                    print(f"SKIP step0: {path} -> {reason}")
    ...
    print("Step 3b: score diagnostics and trimming (max |w|)")
    if args.winsorize:
    ...
    if args.trim_frac > 0 and apply_quantile is not None:
        print("Note: --apply-quantile ignored because --trim-frac > 0.")
        apply_quantile = None
    ...
    if args.trim_frac > 0 and finite_scores.size > 0:
        n_trim = int(np.floor(args.trim_frac * total_good))
        keep_mask = np.ones(total_good, dtype=bool)
        if n_trim > 0:
            trim_order = np.argsort(score_sort_key)
            trim_idx = trim_order[-n_trim:]
            keep_mask[trim_idx] = False
        print(f"Spike trimming applied: trim_frac={args.trim_frac:.6f}")
    elif apply_quantile is not None and finite_scores.size > 0:
    ...
    trimmed_report_path = os.path.join(out_dir, "trimed_samples.txt")
    with open(trimmed_report_path, "w", encoding="utf-8") as handle:
        handle.write("path\n")
        for path, keep in zip(good_paths, keep_mask):
            if not keep:
                handle.write(f"{path}\n")
    print(f"Wrote trimmed samples list: {trimmed_report_path}")
